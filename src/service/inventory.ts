...
/** Count total items by id across all stacks. */
export function count(inventory: ReadonlyArray<InventoryEntry>, id: ItemId): number {
  let total = 0;
...
/**
 * Add items respecting per-item stack limits.
 * Fills existing stacks first, then creates new stacks as needed.
 */
export function add(
...
/** Consume up to amount of items across stacks, left-to-right. */
