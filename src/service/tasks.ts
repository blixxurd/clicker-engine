/**
 * Task/Quest services: evaluate unlocks/status and claim rewards.
 * Pure functions; side-effect free except returning new state and events.
 */
...
function getTask(defs: Registries["tasks"], id: TaskInstance["id"]): TaskDefinition | undefined {
  return defs?.get(id);
}

/**
 * Evaluate task instances for unlocks and status transitions.
 * @example
 * const { state, events } = evaluateTasks(state, registries);
 */
export function evaluateTasks(state: Readonly<GameState>, registries: Registries): { state: GameState; events: ReadonlyArray<EngineEvent> } {
  ...
  for (const inst of current) {
    const def = getTask(defs, inst.id);
    if (!def) { next.push(inst); continue; }
    // cooldown tick-down (dt applied by caller in a future enhancement)
    if (inst.cooldownRemainingSeconds && inst.cooldownRemainingSeconds > 0) {
      const remaining = inst.cooldownRemainingSeconds; // unchanged in this pass
      next.push({ ...inst, cooldownRemainingSeconds: remaining });
      continue;
    }
    if (inst.status === "locked") {
      if (requirementsMet(state, def)) {
  ...
  // Include instances for any definitions not yet tracked
  if (defs.all) {
    for (const d of defs.all()) {
      if (!current.find((t) => t.id === d.id)) {
        const status = requirementsMet(state, d) ? "active" : "locked";
...
function requirementsMet(state: Readonly<GameState>, def: TaskDefinition): boolean {
...
/**
 * Claim a task's rewards and update instance status.
 * @example
 * const { state, events } = claimTask(state, taskId, registries);
 */
export function claimTask(state: Readonly<GameState>, taskId: TaskInstance["id"], registries: Registries): { state: GameState; events: ReadonlyArray<EngineEvent> } {
  const defs = registries.tasks;
  if (!defs) return { state: state as GameState, events: [] };
  ...
  for (const reward of def.rewards) {
    if (reward.kind === "grantResource") {
      ...
      if (idx >= 0) {
        ...
        const newAmt = q((current.amount as unknown as number) + (reward.amount as unknown as number));
        next = { ...next, resources: next.resources.map((r, i) => (i === idx ? { ...r, amount: newAmt } : r)) } as GameState;
      }
    } else if (reward.kind === "grantItem") {
      next = { ...next, inventory: invAdd(next.inventory, reward.itemId, reward.count, registries.items) } as GameState;
    }
  }
